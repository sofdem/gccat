...
ctr_date(lex_greater,['20030820','20040530','20060811']).

ctr_origin(lex_greater, '\\index{CHIP|indexuse}CHIP', []).

ctr_arguments(lex_greater,
              ['VECTOR1'-collection(var-dvar),
               'VECTOR2'-collection(var-dvar)]).

ctr_exchangeable(lex_greater,
                 [vals(['VECTOR1'^var],int,<,dontcare,dontcare),
                  vals(['VECTOR2'^var],int,>,dontcare,dontcare)]).

ctr_synonyms(lex_greater, [lex, lex_chain, rel, greater, gt]).

ctr_restrictions(lex_greater,
                 [required('VECTOR1',var)          ,
                  required('VECTOR2',var)          ,
                  size('VECTOR1') = size('VECTOR2')]).

ctr_typical(lex_greater,
            [size('VECTOR1') > 1                                                                                           ,
...
ctr_extensible(lex_greater, [], ['VECTOR1','VECTOR2'], suffix).

ctr_derived_collections(lex_greater,
...
ctr_example(lex_greater,
            lex_greater([[var-5], [var-2], [var-7], [var-1]],
                        [[var-5], [var-2], [var-6], [var-2]])).

ctr_draw_example(lex_greater,
                 ['COMPONENTS','DESTINATION'],
                 [[[index-1, x-5, y-5],
                   [index-2, x-2, y-2],
                   [index-3, x-7, y-6],
...
ctr_see_also(lex_greater,
 [link('negation',                    lex_lesseq,          '',    []),
  link('system of constraints',       lex_chain_greater,   '',    []),
  link('implies',                     lex_greatereq,       '',    []),
  link('implies',                     lex_different,       '',    []),
  link('implies (if swap arguments)', lex_less,            '',    []),
  ...
  link('common keyword',              cond_lex_greater,    '%k',  ['lexicographic order'])]).

ctr_key_words(lex_greater,['order constraint'                       ,
                           'vector'                                 ,
                           'symmetry'                               ,
                           'matrix symmetry'                        ,
                           'lexicographic order'                    ,
...
ctr_persons(lex_greater,['Frisch A. M.'       ,
                         'Hnich B.'           ,
                         'K{\\i}z{\\i}ltan Z.',
                         'Miguel I.'          ,
                         'Walsh T.'           ,
...
ctr_eval(lex_greater, [checker(lex_greater_c)  ,
		       builtin(lex_greater_b)  ,
                       automaton(lex_greater_a)]).

lex_greater_c(VECTOR1, VECTOR2) :-
    ...
    lex_greater_c1(VECT1, VECT2).

lex_greater_c1([V|R], [V|S]) :- !,
    lex_greater_c1(R, S).
lex_greater_c1([V1|_], [V2|_]) :-
    V1 > V2.

lex_greater_b(VECTOR1, VECTOR2) :-
...
% 2: VAR1 = VAR2
% 3: VAR1 > VAR2
lex_greater_a(FLAG, VECTOR1, VECTOR2) :-
    collection(VECTOR1, [dvar]),
    collection(VECTOR2, [dvar]),
    length(VECTOR1, L),
    length(VECTOR2, L),
    lex_greater_signature(VECTOR1, VECTOR2, SIGNATURE),
...
lex_greater_signature([[var-VAR1]|Xs], [[var-VAR2]|Ys], [S|Ss]) :-
    S in 1..3,
    VAR1 #< VAR2 #<=> S #= 1,
    VAR1 #= VAR2 #<=> S #= 2,
    VAR1 #> VAR2 #<=> S #= 3,
    lex_greater_signature(Xs, Ys, Ss).
