...
ctr_typical(elements,
            [size('ITEMS')        > 1,
             range('ITEMS'^index) > 1,
             size('TABLE')        > 1,
             range('TABLE'^value) > 1]).

ctr_pure_functional_dependency(elements, []).
ctr_functional_dependency(elements, 1-2, [1-1,2]).
...
ctr_example(elements,
            elements([[index-4, value-9],
                      [index-1, value-6]],
                     [[index-1, value-6],
                      [index-2, value-9],
                      [index-3, value-2],
...
ctr_cond_imply(elements, bin_packing_capa, [distinct('ITEMS',index), 'TABLE'^value >= 0], [], ['TABLE','ITEMS']).

ctr_see_also(elements,
 [link('implied by',                    elem,                  '', []),
  link('implied by',                    elements_alldifferent, '', []),
  link('part of system of constraints', element,               '', []),
  link('part of system of constraints', elem,                  '', [])]).

ctr_key_words(elements,
              ['data constraint'           ,
...
ctr_eval(elements, [reformulation(elements_r)]).

elements_r(ITEMS, TABLE) :-
    length(TABLE, N),
    ...
    elements1(INDS, VALS, VALUES).

elements1([], [], _).
elements1([IND|R], [VAL|S], VALUES) :-
    element(IND, VALUES, VAL),
    elements1(R, S, VALUES).
