...
ctr_date(clique,['20030820','20040530','20060805']).

ctr_origin(clique, '\\cite{Fahle02}', []).

ctr_arguments(clique,
              ['SIZE_CLIQUE'-dvar                      ,
               'NODES'-collection(index-int, succ-svar)]).

ctr_exchangeable(clique,
                 [items('NODES',all)]).

ctr_restrictions(clique,
                 ['SIZE_CLIQUE' >= 0            ,
...
ctr_functional_dependency(clique, 1, [2]).
...
ctr_see_also(clique,
 [link('common keyword',            link_set_to_booleans, '%k, can be used for channelling', ['constraint involving set variables']),
  link('used in graph description', in_set,               '',                                [])]).

ctr_key_words(clique,['graph constraint'                  ,
                      'maximum clique'                    ,
                      'constraint involving set variables',
                      'symmetric'                         ,
                      'functional dependency'             ]).

ctr_persons(clique,['Fahle T.'       ,
                    'R\\\'egin J.-C.',
                    'Bron C.'        ,
                    'Kerbosch J.'    ]).

ctr_application(clique, [2]).
