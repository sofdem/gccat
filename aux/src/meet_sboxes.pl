...
ctr_date(meet_sboxes,['20070622','20090725']).

ctr_origin(meet_sboxes,'Geometry, derived from \\cite{RandellCuiCohn92}', []).

ctr_types(meet_sboxes,
          ['VARIABLES'-collection(v-dvar),
           'INTEGERS'-collection(v-int)  ,
           'POSITIVES'-collection(v-int) ]).

ctr_arguments(meet_sboxes,
              ['K'-int                                                  ,
               'DIMS'-sint                                              ,
               'OBJECTS'-collection(oid-int, sid-dvar, x-'VARIABLES')   ,
               'SBOXES'-collection(sid-int, t-'INTEGERS', l-'POSITIVES')]).

ctr_exchangeable(meet_sboxes,
                 [items('OBJECTS',all),
                  items('SBOXES',all),
                  items_sync('OBJECTS'^x,'SBOXES'^t,'SBOXES'^l,all)]).

ctr_synonyms(meet_sboxes,[meet]).

ctr_restrictions(meet_sboxes,
                 [size('VARIABLES') >= 1              ,
                  size('INTEGERS')  >= 1              ,
                  size('POSITIVES') >= 1              ,
...
ctr_typical(meet_sboxes,
            [size('OBJECTS') > 1]).

ctr_contractible(meet_sboxes, [], 'OBJECTS', suffix).

ctr_example(meet_sboxes,
            meet_sboxes(2,
                        {0,1},
                        [[oid-1,sid-1,x-[[v-3],[v-2]]],
                         [oid-2,sid-2,x-[[v-4],[v-1]]],
                         [oid-3,sid-4,x-[[v-3],[v-4]]]],
...
ctr_key_words(meet_sboxes,['logic'                 ,
                           'geometrical constraint',
                           'rcc8'                  ]).

ctr_persons(meet_sboxes,['Randell D. A.',
                         'Cui Z.'       ,
                         'Cohn A. G.'   ]).

ctr_application(meet_sboxes, [3]).

ctr_eval(meet_sboxes, [logic(meet_sboxes_g)]).

ctr_logic(meet_sboxes,
	  [DIMENSIONS, OIDS],
	   ...
	   (all_meet(Dims, OIDS) --->
	    forall(O1,objects(OIDS),
		   forall(O2,objects(OIDS),
			  (O1^oid #< O2^oid #=> meet_objects(Dims, O1, O2))))),
	   all_meet(DIMENSIONS, OIDS)]).

meet_sboxes_g(K, _, [], _) :-
    !,
    check_type(int_gteq(1), K).
meet_sboxes_g(K, _DIMS, OBJECTS, SBOXES) :-
    length(OBJECTS, O),
    length(SBOXES, S),
    O > 0,
    S > 0,
    ...
    ctr_logic(meet_sboxes, [DIMENSIONS, OIDS], Rules),
