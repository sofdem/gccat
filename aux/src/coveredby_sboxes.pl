...
ctr_date(coveredby_sboxes,['20070622','20090725']).

ctr_origin(coveredby_sboxes,'Geometry, derived from \\cite{RandellCuiCohn92}', []).

ctr_types(coveredby_sboxes,
          ['VARIABLES'-collection(v-dvar),
           'INTEGERS'-collection(v-int)  ,
           'POSITIVES'-collection(v-int) ]).

ctr_arguments(coveredby_sboxes,
              ['K'-int                                                  ,
               'DIMS'-sint                                              ,
               'OBJECTS'-collection(oid-int, sid-dvar, x-'VARIABLES')   ,
               'SBOXES'-collection(sid-int, t-'INTEGERS', l-'POSITIVES')]).

ctr_exchangeable(coveredby_sboxes,
                 [items('SBOXES',all),
                  items_sync('OBJECTS'^x,'SBOXES'^t,'SBOXES'^l,all)]).

ctr_synonyms(coveredby_sboxes,[coveredby]).

ctr_restrictions(coveredby_sboxes,
                 [size('VARIABLES') >= 1              ,
                  size('INTEGERS')  >= 1              ,
                  size('POSITIVES') >= 1              ,
...
ctr_typical(coveredby_sboxes,
            [size('OBJECTS') > 1]).

ctr_example(coveredby_sboxes,
            coveredby_sboxes(2,
                             {0,1},
                             [[oid-1,sid-4,x-[[v-2],[v-3]]],
                              [oid-2,sid-2,x-[[v-2],[v-2]]],
                              [oid-3,sid-1,x-[[v-1],[v-1]]]],
...
ctr_key_words(coveredby_sboxes,['logic'                 ,
                                'geometrical constraint',
                                'rcc8'                  ,
                                'obscure'               ]).

ctr_persons(coveredby_sboxes,['Randell D. A.',
                              'Cui Z.'       ,
                              'Cohn A. G.'   ]).

ctr_application(coveredby_sboxes, [3]).

ctr_eval(coveredby_sboxes, [logic(coveredby_sboxes_g)]).

ctr_logic(coveredby_sboxes,
	  [DIMENSIONS, OIDS],
	  [(origin(O1,S1,D) ---> O1^x(D)+S1^t(D)),
	   ...
	   (coveredby_objects(Dims, O1, O2) --->
		      ...
		      exists(S2, sboxes([O2^sid]),
			     coveredby_sboxes(Dims, O1, S1, O2, S2)))),
	   (all_coveredby(Dims, OIDS) --->
	       forall(O1,objects(OIDS),
		      forall(O2,objects(OIDS),
			     (O1^oid #< O2^oid #=> coveredby_objects(Dims, O1, O2))))),
	   all_coveredby(DIMENSIONS, OIDS)]).

    coveredby_sboxes_g(K, _, [], _) :-
    !,
    check_type(int_gteq(1), K).
coveredby_sboxes_g(K, _DIMS, OBJECTS, SBOXES) :-
    length(OBJECTS, O),
    length(SBOXES, S),
    O > 0,
    S > 0,
    ...
    ctr_logic(coveredby_sboxes, [DIMENSIONS, OIDS], Rules),
