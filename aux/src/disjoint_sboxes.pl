...
ctr_date(disjoint_sboxes,['20070622','20090725']).

ctr_origin(disjoint_sboxes,'Geometry, derived from \\cite{RandellCuiCohn92}', []).

ctr_types(disjoint_sboxes,
          ['VARIABLES'-collection(v-dvar),
           'INTEGERS'-collection(v-int)  ,
           'POSITIVES'-collection(v-int) ]).

ctr_arguments(disjoint_sboxes,
              ['K'-int                                                  ,
               'DIMS'-sint                                              ,
               'OBJECTS'-collection(oid-int, sid-dvar, x-'VARIABLES')   ,
               'SBOXES'-collection(sid-int, t-'INTEGERS', l-'POSITIVES')]).

ctr_exchangeable(disjoint_sboxes,
                 [items('OBJECTS',all),
                  items('SBOXES',all),
                  vals(['SBOXES'^l^v],int(>=(1)),>,dontcare,dontcare)]).

ctr_typical(disjoint_sboxes,
            [size('OBJECTS') > 1]).

ctr_synonyms(disjoint_sboxes,[disjoint]).

ctr_restrictions(disjoint_sboxes,
                 [size('VARIABLES') >= 1              ,
                  size('INTEGERS')  >= 1              ,
                  size('POSITIVES') >= 1              ,
...
ctr_contractible(disjoint_sboxes, [], 'OBJECTS', suffix).

ctr_example(disjoint_sboxes,
            disjoint_sboxes(2,
                            {0,1},
                            [[oid-1,sid-1,x-[[v-1],[v-1]]],
                             [oid-2,sid-2,x-[[v-4],[v-1]]],
                             [oid-3,sid-4,x-[[v-2],[v-4]]]],
...
ctr_see_also(disjoint_sboxes,
 [link('implies',        non_overlap_sboxes, '',      []),
...
ctr_key_words(disjoint_sboxes,['logic'                 ,
                               'geometrical constraint',
                               'rcc8'                  ,
                               'obscure'               ]).

ctr_persons(disjoint_sboxes,['Randell D. A.',
                             'Cui Z.'       ,
                             'Cohn A. G.'   ]).

ctr_application(disjoint_sboxes, [3]).

ctr_eval(disjoint_sboxes, [logic(disjoint_sboxes_g)]).

ctr_logic(disjoint_sboxes,
	  [DIMENSIONS, OIDS],
	   ...
	   (disjoint_objects(Dims, O1, O2) --->
	    forall(S1, sboxes([O1^sid]),
		   forall(S2, sboxes([O2^sid]),
			  disjoint_sboxes(Dims, O1, S1, O2, S2)))),
	   (all_disjoint(Dims, OIDS) --->
	       forall(O1,objects(OIDS),
		      forall(O2,objects(OIDS),
			     (O1^oid #< O2^oid #=> disjoint_objects(Dims, O1, O2))))),
	   all_disjoint(DIMENSIONS, OIDS)]).

disjoint_sboxes_g(K, _, [], _) :-
    !,
    check_type(int_gteq(1), K).
disjoint_sboxes_g(K, _DIMS, OBJECTS, SBOXES) :-
    length(OBJECTS, O),
    length(SBOXES, S),
    O > 0,
    S > 0,
    ...
    ctr_logic(disjoint_sboxes, [DIMENSIONS, OIDS], Rules),
