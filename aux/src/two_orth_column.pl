...
ctr_origin(two_orth_column, 'Used for defining %c.', [diffn_column]).

ctr_types(two_orth_column,
          ['ORTHOTOPE'-collection(ori-dvar, siz-dvar, end-dvar)]).

ctr_arguments(two_orth_column,
              ['ORTHOTOPE1'-'ORTHOTOPE',
               'ORTHOTOPE2'-'ORTHOTOPE',
               'DIM'-int               ]).

ctr_exchangeable(two_orth_column,
                 [args([['ORTHOTOPE1','ORTHOTOPE2'],['DIM']])]).

ctr_restrictions(two_orth_column,
                 [size('ORTHOTOPE') > 0                        ,
...
ctr_typical(two_orth_column,
            [size('ORTHOTOPE') > 1]).

ctr_graph(two_orth_column,
          ...
          [(orthotope1^key = 'DIM'          #/\
            ...
            #=>
           (min(orthotope1^end,orthotope2^end)-max(orthotope1^ori,orthotope2^ori) = orthotope1^siz #/\
            orthotope1^siz = orthotope2^siz
           )],
          ['NARC' = 1],
          []).

ctr_example(two_orth_column,
            two_orth_column([[ori-1,siz-3,end-4], [ori-1,siz-1,end-2]],
                            [[ori-4,siz-2,end-6], [ori-1,siz-3,end-4]],
                            1)).

ctr_draw_example(two_orth_column,
                 ['ORTHOTOPE1','ORTHOTOPE2'],
...
ctr_see_also(two_orth_column,
 [link('implies', two_orth_include, '',                                  []),
  link('related', diffn,            'an extension of the %c constraint', [diffn])]).

ctr_key_words(two_orth_column,['logic'                 ,
			       'geometrical constraint',
                               'positioning constraint',
                               'orthotope'             ,
                               'guillotine cut'        ]).

ctr_eval(two_orth_column, [reformulation(two_orth_column_r)]).

two_orth_column_r(ORTHOTOPE1, ORTHOTOPE2, DIM) :-
    ...
    O1 #< E2 #/\ O2 #< E1 #/\ S1 #> 0 #/\ S2 #> 0 #=> min(E1,E2)-max(O1,O2) #= S1 #/\ S1 #= S2.
