...
ctr_date(contains_sboxes,['20070622','20090725']).

ctr_origin(contains_sboxes,'Geometry, derived from \\cite{RandellCuiCohn92}', []).

ctr_types(contains_sboxes,
          ['VARIABLES'-collection(v-dvar),
           'INTEGERS'-collection(v-int)  ,
           'POSITIVES'-collection(v-int) ]).

ctr_arguments(contains_sboxes,
              ['K'-int                                                  ,
               'DIMS'-sint                                              ,
               'OBJECTS'-collection(oid-int, sid-dvar, x-'VARIABLES')   ,
               'SBOXES'-collection(sid-int, t-'INTEGERS', l-'POSITIVES')]).

ctr_exchangeable(contains_sboxes,
                 [items('SBOXES',all),
                  items_sync('OBJECTS'^x,'SBOXES'^t,'SBOXES'^l,all)]).

ctr_synonyms(contains_sboxes,[contains]).

ctr_restrictions(contains_sboxes,
                 [size('VARIABLES') >= 1              ,
                  size('INTEGERS')  >= 1              ,
                  size('POSITIVES') >= 1              ,
...
ctr_typical(contains_sboxes,
            [size('OBJECTS') > 1]).

ctr_contractible(contains_sboxes, [], 'OBJECTS', suffix).

ctr_example(contains_sboxes,
            contains_sboxes(2,
                            {0,1},
                            [[oid-1,sid-1,x-[[v-1],[v-1]]],
                             [oid-2,sid-2,x-[[v-2],[v-2]]],
                             [oid-3,sid-3,x-[[v-3],[v-3]]]],
                            [[sid-1,t-[[v-0],[v-0]],l-[[v-5],[v-5]]],
                             [sid-2,t-[[v-0],[v-0]],l-[[v-3],[v-3]]],
                             [sid-3,t-[[v-0],[v-0]],l-[[v-1],[v-1]]]])).

ctr_see_also(contains_sboxes,
...
ctr_key_words(contains_sboxes,['logic'                 ,
                               'geometrical constraint',
                               'rcc8'                  ,
                               'obscure'               ]).

ctr_persons(contains_sboxes,['Randell D. A.',
                             'Cui Z.'       ,
                             'Cohn A. G.'   ]).

ctr_application(contains_sboxes, [3]).

ctr_eval(contains_sboxes, [logic(contains_sboxes_g)]).

ctr_logic(contains_sboxes,
	  [DIMENSIONS, OIDS],
	   ...
	   (contains_objects(Dims, O1, O2) --->
	       forall(S1, sboxes([O1^sid]),
		      exists(S2, sboxes([O2^sid]),
			     contains_sboxes(Dims, O1, S1, O2, S2)))),
	   (all_contains(Dims, OIDS) --->
	       forall(O1,objects(OIDS),
		      forall(O2,objects(OIDS),
			     (O1^oid #< O2^oid #=> contains_objects(Dims, O1, O2))))),
	   all_contains(DIMENSIONS, OIDS)]).


contains_sboxes_g(K, _, [], _) :-
    !,
    check_type(int_gteq(1), K).
contains_sboxes_g(K, _DIMS, OBJECTS, SBOXES) :-
    length(OBJECTS, O),
    length(SBOXES, S),
    O > 0,
    S > 0,
    ...
    ctr_logic(contains_sboxes, [DIMENSIONS, OIDS], Rules), 
