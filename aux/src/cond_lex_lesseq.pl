...
ctr_date(cond_lex_lesseq,['20060416']).

ctr_origin(cond_lex_lesseq, 'Inspired by \\cite{WallaceWilson06}.', []).

ctr_types(cond_lex_lesseq, ['TUPLE_OF_VALS'-collection(val-int)]).

ctr_arguments(cond_lex_lesseq,
              ['VECTOR1'-collection(var-dvar)                      ,
               'VECTOR2'-collection(var-dvar)                      ,
               'PREFERENCE_TABLE'-collection(tuple-'TUPLE_OF_VALS')]).

ctr_exchangeable(cond_lex_lesseq,
                 [items_sync('VECTOR1','VECTOR2','PREFERENCE_TABLE'^tuple,all),
                  vals(['VECTOR1','VECTOR2','PREFERENCE_TABLE'^tuple],int,=\=,all,dontcare)]).

ctr_restrictions(cond_lex_lesseq,
...
ctr_typical(cond_lex_lesseq,
            [size('TUPLE_OF_VALS')    > 1,
             size('VECTOR1')          > 1,
             size('VECTOR2')          > 1,
             size('PREFERENCE_TABLE') > 1]).

ctr_example(cond_lex_lesseq,
            cond_lex_lesseq([[var-1], [var-0]],
                            [[var-0], [var-0]],
                            [[tuple-[[val-1], [val-0]]],
                             [tuple-[[val-0], [val-1]]],
                             [tuple-[[val-0], [val-0]]],
...
ctr_see_also(cond_lex_lesseq,
 [link('implied by',     cond_lex_less,      '',   []),
...
ctr_key_words(cond_lex_lesseq,['order constraint'                ,
                               'vector'                          ,
                               'lexicographic order'             ,
                               'preferences'                     ,
...
ctr_persons(cond_lex_lesseq,['Wallace R. J.',
                             'Wilson N.'    ]).

ctr_eval(cond_lex_lesseq, [automata(cond_lex_lesseq_a)]).

cond_lex_lesseq_a(VECTOR1, VECTOR2, PREFERENCE_TABLE) :-
