...
ctr_date(gt,['20070821']).

ctr_origin(gt, 'Arithmetic.', []).
...
ctr_see_also(gt,
 [link('negation',                    leq, '',      []),
  link('implies',                     neq, '',      []),
  link('implies',                     geq, '',      []),
  link('implies (if swap arguments)', lt,  '',      []),
  link('common keyword',              eq,  '%k,%k', ['binary constraint', 'arithmetic constraint'])]).

ctr_key_words(gt,['predefined constraint',
                  'arithmetic constraint',
                  'binary constraint'    ,
                  'arc-consistency'      ]).

ctr_eval(gt, [builtin(gt_b)]).
